...
class Generator(nn.Module):
    def __init__(self, vocabs, word_char_dim, word_dim, concept_char_dim,
            concept_dim, cnn_filters, char2word_dim, char2concept_dim, rel_dim,
            rnn_hidden_size, rnn_num_layers, embed_dim, ff_embed_dim, num_heads,
            dropout, snt_layers, graph_layers, inference_layers,
        ...
        self.bag_encoder = BagEncoder(vocabs['relation'], rel_dim, embed_dim,
                char2word_dim, rnn_hidden_size, rnn_num_layers, dropout, num_heads)
        self.token_encoder = TokenEncoder(vocabs['token'], vocabs['token_char'],
                        word_char_dim, word_dim, embed_dim, cnn_filters,
                        char2word_dim, dropout, pretrained_file)
    ...
    def con_rel_enc(self, inp):
        ...
        attn_mask = torch.eq(inp['relation'], self.vocabs['relation'].padding_idx)
        bad = attn_mask.all(1)
        if bad.any():
            nz = torch.nonzero(bad)
            attn_mask[nz[:, 0], 0, nz[:, 1]] = 0
        relation = self.bag_encoder(inp['relation'], inp['d2ns'], inp['dts'],
                inp['dv2rels'], inp['d2ns_r'], inp['dts_r'], inp['bag_rels'],
                inp['bis'], inp['bds'], inp['bds_r'], inp['sgs'])
