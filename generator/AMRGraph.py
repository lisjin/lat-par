...
class AMRGraph(object):
    ...
    def collect_concepts_and_relations(self, edges_only=True):
        g = self.graph
        nodes, depths, is_connected = self.bfs()
        concepts = [self.name2concept[n] for n in nodes]
        relations = self.collect_edges(g, nodes) if edges_only else\
                self.collect_relations(g, nodes)
        return concepts, depths, relations, is_connected

class Molecule(AMRGraph):
    def __init__(self, nodes_in, edge_list, nfeat, efeat):
        self.graph = nx.DiGraph()
        self.edge_list = edge_list
        self.nfeat = nfeat
        self.efeat = efeat
        for e in edge_list:
            self._add_edge('1', e[0], e[1])

        if self.graph.order():
            self.root = min(list(self.graph.nodes))
            self.nodes, _, _ = self.bfs()
            rem = set(nodes_in) - set(self.nodes)
            for u in rem:
                self.graph.add_node(u)
                self.nodes.append(u)

            n_idx = np.array(self.nodes)
            self.nfeat = self.nfeat[n_idx]
            assert(self.nfeat.shape[0] == len(self.nodes))

            n_mp = {v: k for k, v in enumerate(n_idx)}
            self.edge_list = np.array([(n_mp[e[0]], n_mp[e[1]]) for e in edge_list])
