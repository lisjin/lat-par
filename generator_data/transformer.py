...
class Transformer(jit.ScriptModule):
    ...
    def forward(self, x, kv = None,
        ...
        # type: (Tensor, Optional[Tensor], Optional[Tensor], Optional[Tensor], Optional[Tensor], Optional[Tensor]) -> Tensor
        for idx, layer in enumerate(self.layers):
            x, _, _ = layer(x, kv, self_padding_mask, self_attn_mask, external_memories, external_padding_mask)
        return x

class TransformerLayer(jit.ScriptModule):
    ...
    def forward(self, x, kv = None,
                ...
                external_memories = None, external_padding_mask=None,
                need_weights = False):
        # type: (Tensor, Optional[Tensor], Optional[Tensor], Optional[Tensor], Optional[Tensor], Optional[Tensor], bool) -> Tuple[Tensor, Tensor, Optional[Tensor]]
        # x: seq_len x bsz x embed_dim
        residual = x
        if kv is None:
            x, self_attn = self.self_attn(query=x, key_padding_mask=self_padding_mask, attn_mask=self_attn_mask, need_weights=need_weights)
        else:
            x, self_attn = self.self_attn(query=x, key=kv, key_padding_mask=self_padding_mask, attn_mask=self_attn_mask, need_weights=need_weights)
        ...
        if self.with_external:
            residual = x
            x, external_attn = self.external_attn(query=x, key=external_memories, value=None, key_padding_mask=external_padding_mask, need_weights=need_weights)
            x = F.dropout(x, p=self.dropout, training=self.training)
...
class MultiheadAttention(jit.ScriptModule):
    ...
    @jit.script_method
    def forward(self, query, key=None, value=None, key_padding_mask=None, attn_mask=None, fine_attn_mask=None, need_weights=False):
        # type: (Tensor, Optional[Tensor], Optional[Tensor], Optional[Tensor], Optional[Tensor], Optional[Tensor], bool) -> Tuple[Tensor, Tensor]
        ...
        if fine_attn_mask is not None:
            attn_weights = attn_weights.view(bsz, self.num_heads, tgt_len, src_len)
            attn_weights.masked_fill_(
                fine_attn_mask.unsqueeze(1),
                float('-inf')
            )
            attn_weights = attn_weights.view(bsz * self.num_heads, tgt_len, src_len)
