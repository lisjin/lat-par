...
def add_forest_tensors(relation_vocab, d, d2j, padding_idx, self_idx, k, max_nn=1e4):
    def build_dt(gr, pr):
    ...
    def collect_bags(rel_type, sep2frag, n_cur):
        sg = []
        bag_rel = []
        bag_v = []
        ...
        for sep_id, j in b2bi.items():
            ...
            ix = np.ix_(bag, bag)
            bag_rel.append([r for r in rel_type[ix].flatten() if r != self_idx])
            bag_v.append(bag)
        return bi, sg, bag_rel, bag_v

    def dt2mats(dt, n2par, n2d, max_d_a, pr, bi, fill_val=-1, b_up=False, rel_type=None):
        ...
        def chi_rels(chi, par):
            ...
            if chi < len(pr):
                ...
                out |= set(t for t in product(chi_v, repeat=2) if not (t[0] in\
                        xv and t[1] in xv) and rel_type[t[0], t[1]] !=\
                        padding_idx or t[0] == t[1] and t[0] not in par_v)
            return out
    ...
    if 0 < len(pr) and len(pr) < max_nn:
        ...
        n_cur = len(d['concept']) + 1
        bi, sg, bag_rel, bag = collect_bags(d['rel_type'], d['sep2frag'], n_cur)
        d2n, dt, dv2rel, d['max_nl'], d['max_v'], d['max_u'] = dt2mats(dt,
                n2par, n2d, max_d_a, pr, bi, b_up=True, rel_type=d['rel_type'])
...
def get_forest_mats(data, n_conc, relation_vocab, k=4):
    ...
    def dcts2mat(dcts, dtype=torch.short):
        ...
        if len(inds):
            ...
            m = torch.sparse.ShortTensor(inds, torch.tensor(vals))
        else:
            m = torch.zeros((0, 2), dtype=dtype).to_sparse()
        return custom_sparse(m)

    def to_maybe_sparse(m, val, sp):
        m = torch.from_numpy(m) + int(val < 0)
        if sp:
            m = custom_sparse(m.to_sparse())
        return m
    ...
    padding_idx = relation_vocab.token2idx(PAD)
    ...
    for i, d in enumerate(data):
        d2n, dt, dv2rel, d2n_r, dt_r, bag_rel, bi, n2d, n2d_r, sg =\
                add_forest_tensors(relation_vocab, d, d2j, padding_idx, self_idx, k)
    ...
    bag_rels = lsts2mat_3d(bag_rels, (bsz, max_b_a, k * (k - 1)), dtype=np.int)
...
def batchify(data, vocabs, unk_rate=0., is_train=True):
    ...
    d2ns, dts, dv2rels, d2ns_r, dts_r, bag_rels, bis, bds, bds_r, sgs =\
            get_forest_mats(data, _conc.shape[0], vocabs['relation'])
    _relation_type = ArraysToTensor([d['rel_type'] for d in data]).transpose_(0, 2)
...
class MyDataset(IterableDataset):
    def __init__(self, vocabs, lex_map, filename, forests_path, sep2frags_path,
        ...
        for i, d in enumerate(self.data):
            d['cp_seq'], d['token2idx'], d['idx2token'] = lex_map.get(
                    d['concept'], vocabs['predictable_token'])
            d['pr'] = hf['prs'][i]
            d['gr'] = hf['grs'][i]
            d['sep2frag'] = sep2frags[i]
            d['rel_type'] = self.build_rel_type(vocabs['relation'], d)
    ...
    @staticmethod
    def build_rel_type(relation_vocab, d):
        n = len(d['concept'])
        brs = [[3] + [1] * n]
        for i in range(n):
            rs = [2]
            adj_dict = d['relation'][str(i)]
            adj_set = set([int(k) for k in adj_dict.keys()])
            for j in range(n):
                if i == j: # self loop
                    edge = relation_vocab.token2idx(SEL)
                elif j in adj_set:
                    edge = relation_vocab.token2idx(adj_dict[str(j)][0]['edge'][0])
                else:
                    edge = relation_vocab.token2idx(PAD)
                rs.append(edge)
...
if __name__ == '__main__':
    ...
    train_data = MyDataset(vocabs, lexical_mapping, args.train_data, args.train_batch_size, for_train=True)
    epoch_idx = 0
    batch_idx = 0
    while True:
        for d in train_data:
